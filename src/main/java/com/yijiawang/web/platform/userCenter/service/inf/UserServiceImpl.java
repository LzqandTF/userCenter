...
public class UserServiceImpl implements UserService {
	...
	public int saveUserProtectInfo(String userId, String payPassword, List<UserProtectQuestionVO> questions) {
		...
		if (currList.size() == 2) {
			...
			for (int i = 0; i < 2; i++) {
				...
				if (!curr.getQuestionId().equals(commit.getQuestionId())
						|| !curr.getUserAnswer().equals(commit.getUserAnswer())) {
					return 1;
				}
			}
		} else {
	...
	private long changeBalance(AccountCheck accountCheck) {
		...
		try {
			...
			logObject.add(
					" 处理流水前 ... 用户:" + userId + " 账户余额为: " + userAccountMapper.selectByUserId(userId).getBalance());
			if (accountCheck.getTradeType() == TradeType.TOPUP.value()) {
				...
				AccountCheck frozenAccountCheck = accountCheckMapper
						.selectFrozenAccountCheck(accountCheck.getOrderId());
				if (frozenAccountCheck != null) {
					logObject.add(" 退款操作 7天无理由退款 ");
					// 有冻结金额,先更新卖家冻结金额
					if (userAccountMapper.updateFrozenMoney2UserAccount(frozenAccountCheck.getUserId(),
							-1 * frozenAccountCheck.getTradeAmount()) > 0) {
						...
						unFrozenCheck.setResultBalance(
								userAccountMapper.selectByUserId(frozenAccountCheck.getUserId()).getBalance());
				...
				if (userAccountMapper.updateBalance2UserAccount(userId, amount) > 0) {
					...
					if (frozenAccountCheck != null) {
						// 已经有冻结的流水,先更新冻结余额
						if (userAccountMapper.updateFrozenMoney2UserAccount(accountCheck.getUserId(),
								-1 * accountCheck.getTradeAmount()) > 0) {
				...
				if (userAccountMapper.updateBalance2UserAccount(userId, amount) > 0) {
					...
					if (accountCheckMapper.insert(accountCheck) > 0) {
						...
						if (updateInsurePriceInfo(accountCheck.getUserId(), accountCheck.getLotId(),
								InsureStatus.REFUND.value()) > 0) {
				...
				if (info != null) {
					if (userAccountMapper.updateBalance2UserAccount(info.getSalerId(), info.getInsurePrice()) > 0) {
						...
						salerAccountCheck.setResultBalance(
								userAccountMapper.selectByUserId(salerWxUserInfo.getUserId()).getBalance());
